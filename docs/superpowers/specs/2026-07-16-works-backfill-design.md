...
日付: 2026-07-16
改訂: 2026-07-16 — 補完の資格判定を解決済み機関IDからraw所属文字列に変更（末尾の改訂記録を参照）
...
### 2026-07-16: 資格判定をraw所属文字列ベースに変更

初版の「authorshipの解決済み機関IDが対象4機関と交差したら補完」は、誤解決の逆方向を
考慮していなかった。実例: 白井こころ氏（大阪大学・公衆衛生）のLancet論文は、raw所属が
"Osaka University" なのにOpenAlexが旧市大IDにも誤解決しており、補完4,582人のうち
1,898人が同種の学外者（多くは阪大医学系）だった。

対策: 資格判定を「raw所属文字列（works.raw_jsonに保存済み）がOMU系表記
（Osaka Metropolitan/City/Prefecture Univ…、大阪公立大学・大阪市立大学・大阪府立大学。
高専は除外）にマッチするauthorshipを持つこと」に変更（`collector/parse.py` の
`omu_raw_author_ids`）。解決済みIDは資格判定には使わない（works取得フィルタでは引き続き使う。
raw文字列はAPIフィルタにできないため）。副次効果として、rawはOMUなのに解決済みIDが
学外になったauthorship（三木氏の2026-01論文のパターン）も資格として拾えるようになった。
`authorships.institution_ids` 列は参考情報として保存を継続する。
